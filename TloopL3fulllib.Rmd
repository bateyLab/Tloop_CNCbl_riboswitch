...
finalmedFI <-lapply(noOut_TloopL3_FI[1:nrow(noOut_TloopL3_FI),1:ncol(noOut_TloopL3_FI)], median, na.rm=TRUE)
finalmedFItest <- as.data.frame(apply(noOut_TloopL3_FI, 2, median, na.rm=TRUE)) 
#make column that identifies colonies as env8 for plotting
```
...
SEL3_plus<- as.data.frame(apply(noOut_TloopL3_plus, 2, mean_se))
#SE_L3_plus <-
``` 
...
SEL3_minus<- as.data.frame(apply(noOut_TloopL3_minus, 2, mean_se))
...
#!!!!!!!!!!!!!!!!!
TloopL3_minusvsenv8_confidence_interval <- TloopL3_minusvsenv8_confidence_interval[order(TloopL3_FIvsenv8_confidence_interval$rowname),]
TloopL3_minus_noOut_wideform <- TloopL3_minus_noOut_wideform[,order(colnames(TloopL3_minus_noOut_wideform))]
#wideform_df <-wideform_df[,order(colnames(wideform_df))]
signifVec <- c("Significance Level", 0, TloopL3_minusvsenv8_confidence_interval$is_sigL3minus)
TloopL3_minus_noOut_wideform <- rbind(TloopL3_minus_noOut_wideform, signifVec)

test_long_TloopL3_minus <- TloopL3_minus_noOut_wideform %>%
...
test_long_TloopL3_minus$`(-)` <- as.numeric(test_long_TloopL3_minus$`(-)`)
test_long_TloopL3_minus$`Significance Level` <- as.numeric(test_long_TloopL3_minus$`Significance Level`)
test_long_TloopL3_minus <- cbind(test_long_TloopL3_minus, rep(1e+05, nrow(test_long_TloopL3_minus)))
test_long_TloopL3_minus <- as.data.frame(test_long_TloopL3_minus)

test_long_TloopL3_minus <- test_long_TloopL3_minus %>%
  group_by(name) %>%
...
ggplot(test_long_TloopL3_minus, aes(x=reorder(test_long_TloopL3_minus[,1], -test_long_TloopL3_minus[,2]), y=test_long_TloopL3_minus[,2]), fill = test_long_TloopL3_minus[,3])+
    geom_boxplot(alpha=0.8)+
   labs(title="T Loop L3 Minus CNCbl", x="Colony", y="Fluorescence")+
   scale_y_log10()+
    theme_classic()+
    geom_text(aes(label = test_long_TloopL3_minus$`Significance Level`), vjust=0) +
    theme(axis.text.x = element_text(face="bold", angle=90), plot.title = element_text(hjust=0.5, face="bold"), axis.text.y = element_text(face="bold"), axis.title.x = element_text(face = "bold"), legend.position = "none")
...
ggplot(long_TloopL3_minus, aes(x=reorder(Colony_Name,-Fluorescence, na.rm=TRUE), y=Fluorescence, fill=is_env8minus_L3))+
   ...
   scale_y_log10()+
    theme_classic()+
    geom_text(aes(label = test_long_TloopL3_minus$`Significance Level`,x= test_long_TloopL3_minus$name, y = test_long_TloopL3_minus$med), vjust=0, nudge_y = 0.4 ) +
    theme(axis.text.x = element_text(face="bold", angle=90), plot.title = element_text(hjust=0.5, face="bold"), axis.text.y = element_text(face="bold"), axis.title.x = element_text(face = "bold"), legend.position = "none")
```